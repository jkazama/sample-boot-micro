...
public class SystemAdminService {
    ...
    public PagingList<AuditActor> findAuditActor(FindAuditActor p) {
        return TxTemplate.of(txm).readOnly().tx(() -> AuditActor.find(rep, p));
    }

    /** イベント監査ログを検索します。 */
    public PagingList<AuditEvent> findAuditEvent(FindAuditEvent p) {
        return TxTemplate.of(txm).readOnly().tx(() -> AuditEvent.find(rep, p));
    }

    /** アプリケーション設定一覧を検索します。 */
    public List<AppSetting> findAppSetting(FindAppSetting p) {
        return TxTemplate.of(txm).readOnly().tx(() -> AppSetting.find(rep, p));
    }
